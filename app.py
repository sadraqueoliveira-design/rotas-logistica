...
# --- 2. ESTILO CSS (Compacto e Seguro) ---
st.markdown("""
<style>
    ...
    .header-box {
        background-color: #004aad; padding: 20px; border-radius: 12px; 
        text-align: center; color: white; margin-bottom: 25px; 
        box-shadow: 0 4px 8px rgba(0,0,0,0.2); border: 2px solid #003380;
    }
    .header-title { font-size: 26px !important; font-weight: 900 !important; margin: 0 !important; text-transform: uppercase;}
    .header-date { font-size: 20px !important; font-weight: bold !important; color: #FFD700 !important; margin-top: 5px !important; }
    
    .driver-card { background-color: #004aad; color: white; padding: 10px; border-radius: 8px; text-align: center; font-weight: bold; font-size: 1.2rem; margin-bottom: 10px;}
    ...
    .time-block { background-color: #f8f9fa; padding: 10px; border-radius: 8px; border-left: 6px solid #004aad; margin-bottom: 5px;}
            ...
            if not res.empty:
                ...
                for i, (idx, row) in enumerate(res.iterrows()):
                    ...
                    ignore_cols = [
                        ...
                        "turno", "filtro", "retorno", "tipo", "total suportes"
                    ]
                    
                    carga_dict = {}
                    for col in df_rotas.columns:
                        col_lower = str(col).lower()
                        # Se não for uma coluna de metadados, assume que é carga
                        if not any(bad in col_lower for bad in ignore_cols):
                            val = str(row.get(col, '')).strip()
                            # Só mostra se tiver valor válido
                            if val and val not in ['0', '0.0', '0,0', 'nan', 'None', '']:
                                clean_name = str(col).replace("Azambuja", "").replace("Total", "").strip()
                                if not clean_name: clean_name = col
